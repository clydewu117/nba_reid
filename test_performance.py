...
def _run_reid_head(reid_head, features, require_classifier=False):
    """
    Run the ReID head components in eval mode.

    Handles heads that include an additional projection (e.g. VideoMAEv2)
    before the BN neck.
    """
    feat = features

    if hasattr(reid_head, "feat_proj"):
        feat = reid_head.feat_proj(feat)

    if hasattr(reid_head, "bottleneck"):
        bn_feat = reid_head.bottleneck(feat)
    else:
        bn_feat = feat

    classifier = getattr(reid_head, "classifier", None)
    if classifier is None:
        if require_classifier:
            raise AttributeError("ReID head does not expose a classifier for evaluation.")
        cls_score = None
    else:
...
def test_classification(cfg, model, test_loader, device):
    ...
    for batch in tqdm(test_loader, desc="Extracting"):
        ...
        features = model.backbone(videos)
        bn_feat, cls_score = _run_reid_head(model.reid_head, features, require_classifier=True)
...
def test_reid(cfg, model, query_loader, gallery_loader, device):
    ...
    for batch in tqdm(query_loader, desc="Query"):
        ...
        features = model.backbone(videos)
        bn_feat, _ = _run_reid_head(model.reid_head, features)
