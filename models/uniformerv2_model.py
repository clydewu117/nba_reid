...
MODEL_PATH = '/users/PAS2985/lei441/unireid'
_MODELS = {
    "ViT-B/16": os.path.join(MODEL_PATH, "ViT-B-16.pt"),
...
class Transformer(nn.Module):
    def __init__(
        ...
        # projection - Modified for ReID: only keep LayerNorm
        self.ln_post = nn.LayerNorm(n_dim)
        if not self.frozen:
    ...
    def forward(self, x):
        ...
        if self.frozen:
            return self.ln_post(cls_token[0, :, :])
        else:
            weight = self.sigmoid(self.balance)
            residual = x.view(L, N, T_down, C)[0].mean(1)  # L, N, T, C
            return self.ln_post((1 - weight) * cls_token[0, :, :] + weight * residual)
...
def uniformerv2_b16(
    ...
    if pretrained:
        ...
        # 如果 ckpt 是带 'model' 或 'state_dict' key 的包装，自动展开
        if isinstance(state_dict, dict):
            if "model" in state_dict:
                print("[Info] Found 'model' key in checkpoint, using its content.")
                state_dict = state_dict["model"]
            elif "state_dict" in state_dict:
                print("[Info] Found 'state_dict' key in checkpoint, using its content.")
