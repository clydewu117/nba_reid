...
class BasketballVideoDataset(Dataset):
    ...
    def __init__(
        ...
        transform=None,
        seed=42,
        sample_start="beginning",
        split_sampling=False,
        use_presplit=False,
    ...
    def _load_data(self):
        ...
        if self.use_presplit:
                ...
                if self.control_20:
                    ...
                    for shot_type in shot_types_to_load:
                        split_path = os.path.join(
                            identity_path, shot_type, split_folder
                        )
                        ...
                        video_files = sorted(
                            [
                                v
                                for v in os.listdir(split_path)
            ...
            pid_to_label = {
                folder: label for label, folder in enumerate(identity_folders)
            }
            ...
            if self.shot_classification:
                print(
                    f"SHOT_CLASSIFICATION: True (auto-sync identities with both shot types)"
                )
    ...
    def _split_random_sampling(self, total_frames):
        ...
        # 前半部分采4帧
        first_half_indices = self._random_segment_sampling_range(
            0, mid_point, num_frames=4
        )

        # 后半部分采12帧
        second_half_indices = self._random_segment_sampling_range(
            mid_point, total_frames, num_frames=12
        )
    ...
    def _split_uniform_sampling(self, total_frames):
        ...
        # 前半部分采4帧
        first_half_indices = self._uniform_segment_sampling_range(
            0, mid_point, num_frames=4
        )

        # 后半部分采12帧
        second_half_indices = self._uniform_segment_sampling_range(
            mid_point, total_frames, num_frames=12
        )
    ...
    def _random_segment_sampling_range(self, start_idx, end_idx, num_frames):
        ...
        if num_pads != num_frames:
            indices = np.concatenate(
                [indices, np.ones(num_pads, dtype=int) * (end_idx - 1)]
            )
...
def build_dataloader(cfg, is_train=True):
    ...
    if (
        is_train
        and hasattr(cfg.DATA, "USE_SAMPLER")
        and cfg.DATA.USE_SAMPLER
        and not shot_classification
    ):
