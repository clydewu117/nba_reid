#!/usr/bin/env python3
"""
Batch Grad-CAM visualization pipeline for VideoMAEv2.

Key features compared to the shell workflow:
- Uses the PyAV-based frame sampling strategy aligned with `nba_reid/data/dataloader.py`.
- Reads a train/test split CSV and processes every video labelled as `test`.
- Saves per-frame overlays as PNG images and stores normalized CAM activations.
- Organizes outputs by model → checkpoint → video type → identity → clip → sampling → method.
- Renders both random-segment and uniform-segment sampling variants for every test clip.
"""
...
def _get_sampling_range(total_frames: int, sample_start: str) -> Tuple[int, int]:
    if sample_start == "middle":
...
def _random_segment_sampling(total_frames: int, num_frames: int) -> List[int]:
    start_idx, end_idx = _get_sampling_range(total_frames, "beginning")
    indices = np.arange(start_idx, end_idx)
    available = len(indices)
...
def instantiate_cam_generator(
    model: torch.nn.Module, target_layer: torch.nn.Module, method: str, finer: bool
):
    if method == "originalcam":
        ...
        generator = SimpleGradCAM(model, target_layer, finer=finer)
    return generator
...
def save_frames_as_png(
    ...
    import cv2  # Local import to avoid hard dependency if not needed elsewhere
...
def write_video_from_png(
    ...
    for frame_path in frame_files:
        ...
        if frame is None:
            logger.warning(f"Unable to read frame {frame_path}, skipping remaining frames.")
            break
...
def ensure_metadata(video_root: Path, meta: Dict[str, str]) -> None:
...
def process_video_entry(
    ...
    sampling_sets = sample_frame_indices(
        total_frames,
        args.frames,
    )
    video_root = (
    ...
    for sampling_name, indices in sampling_sets.items():
        ...
        for method in methods:
            ...
            try:
                ...
                if method == "scorecam":
                    cam_flat = cam_generator.generate_cam(
                        input_tensor_batched, target_id=args.target_id, batch_size=args.scorecam_batch_size
                    )
                else:
                    cam_flat = cam_generator.generate_cam(
            ...
            if cam_flat is None:
                logger.error(f"CAM generation returned None for {video_path} [{method}] ({sampling_name})")
                failure += 1
                continue
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch VideoMAEv2 Grad-CAM generator")
    parser.add_argument("--csv", required=True, type=Path, help="Path to train_test_split CSV")
    parser.add_argument("--checkpoints", required=True, nargs="+", type=Path, help="One or more checkpoint paths")
    parser.add_argument("--config", required=True, type=Path, help="Model config YAML")
    parser.add_argument("--output-root", type=Path, default=Path("./batch_cam_output"), help="Base output directory")
    parser.add_argument("--model-name", type=str, default="VideoMAEv2",
    ...
    parser.add_argument("--skip-video", action="store_true", help="Skip writing mp4/avi videos, keep PNG frames only")
...
def main() -> None:
    ...
    for ckpt_path in args.checkpoints:
        ...
        model = load_model(str(ckpt_path), str(args.config), device=args.device)
