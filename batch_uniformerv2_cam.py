#!/usr/bin/env python3
"""
Batch Grad-CAM visualization pipeline for UniFormerV2 ReID models.

Features:
- Uses PyAV-based frame extraction mirroring the project dataloader.
- Processes every "test" row from a CSV split file.
- Renders both random-segment and uniform-segment sampling variants.
- Supports multiple CAM methods (OriginalCAM, ScoreCAM, GradCAM, etc.).
- Optional BN folding and train-mode toggles for stronger gradients.
...
from run_gradcam_uniformer import load_config, load_model, reshape_cam_3d  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch UniFormerV2 Grad-CAM generator")
    ...
    parser.add_argument("--model-name", type=str, default="UniFormerV2",
    ...
    parser.add_argument("--target-id", type=int, default=-1, help="Target class id; -1 uses argmax")
    parser.add_argument("--scorecam-batch-size", type=int, default=64,
                        help="Batch size for ScoreCAM channel processing")
    parser.add_argument("--bn-folding", action="store_true",
                        help="Enable BN folding for GradCAM variants")
    parser.add_argument("--train-mode", action="store_true",
                        help="Use model.train() for gradient-based CAMs (stronger gradients)")
    ...
    parser.add_argument("--finer", action="store_true",
                        help="Use finer score difference when supported (GradCAM/ScoreCAM)")
...
def main() -> None:
    ...
    for ckpt_path in args.checkpoints:
        ...
        model, _ = load_model(str(ckpt_path), str(args.config), device=args.device)
