...
# Try to import from pytorchvideo, but provide fallbacks if not available
try:
...
# Provide stub implementations for missing functions
def cat_all_gather(tensor):
    """Fallback for cat_all_gather."""
    if dist.is_available() and dist.is_initialized():
...
def get_local_process_group():
    """Fallback for get_local_process_group."""
    return None

def get_local_rank():
    """Fallback for get_local_rank."""
    if dist.is_available() and dist.is_initialized():
        return dist.get_rank()
    return 0

def get_local_size():
    """Fallback for get_local_size."""
    return get_world_size()

def _init_distributed_training(num_gpus, shard_id):
    """Fallback for init_distributed_training."""
    if num_gpus > 1 and dist.is_available():
